...
/* Any registers altered by the function beside r0-r3 must be preserved */
	 ...
	 MOV r2, #0             @ max-värde (du kan också initiera med första elementet om du vill)

loop:
    LDR r1, [r0]           @ Läs nuvarande värde från arrayen
    CMP r1, #0             @ Är det nollterminerat?
    BEQ done               @ Avsluta om vi nått slutet

    CMP r1, r2             @ Jämför nuvarande värde med max
    @ Om r1 > r2, uppdatera max-värdet
	@ ... här ska du lägga in en instruktion för det ...
	MOVGT r2,r1 @ Tilldela ifall r2 är större än r1
    @ Annars gör inget
    ADD r0, r0, #4         @ Gå till nästa element i arrayen (4 steg / i++)
    B loop                 @ Hoppa tillbaka till början av loopen

done:
    MOV r0, r2             @ Flytta max-värdet till returregistret
